                        ...
                        <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
                            <tbody>
                                <tr align="center">
                                    ...
                                    <td bgcolor="#ffffff">书籍名</td>
                                    <td bgcolor="#ffffff">购买数量</td>
                                    <td bgcolor="#ffffff">收货人</td>
                                    <td bgcolor="#ffffff">联系电话</td>
                                    <td bgcolor="#ffffff">收货地址</td>
                                    <td bgcolor="#ffffff">购买状态</td>
                                </tr>
                                <tr v-for="item in solds">
                                    <td align="center" bgcolor="#ffffff"><a class="f6" @click="goDetail(item.id)">{{item.order.order_sn}}</a></td>
                                    <td align="center" bgcolor="#ffffff">{{item.books.name}}</td>
                                    <td align="center" bgcolor="#ffffff">{{item.books_num}}</td>
                                    <td align="center" bgcolor="#ffffff">{{item.order.signer_name}}</td>
                                    <td align="center" bgcolor="#ffffff">{{item.order.singer_mobile}}</td>
                                    <td align="center" bgcolor="#ffffff">{{item.order.signer_address}}</td>
                                    <td v-if="item.order.pay_status == 'PAYING' " align="center" bgcolor="#829aa7">待支付</td>
                                    <td v-if="item.order.pay_status == 'TRADE_SUCCESS' " align="center" bgcolor="#11dd35">已支付</td>
                                </tr>
                            </tbody>
                        </table>
...
<script>
  import { getSolds, delSold,getSoldDetail } from '../../api/api'
    export default {
        data () {
            return {
                solds: [
        ...
        methods: {
            ...
            goDetail (id) {
                this.$router.push({name: 'soldDetail', params: {soldId: id}});
            }
        }
    }
</script>
